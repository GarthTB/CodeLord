...
namespace CodeLord.Components
{
    internal static class Encoder
    {
        ...
        public static void Encode(int limit, ConcurrentDictionary<string, List<string>> dict, string text, int codeID)
        {
            try
            {
                var tree = FindBranches(limit, dict, text);
        ...
        private static ConcurrentBag<(int, int, string)> FindBranches(int limit, ConcurrentDictionary<string, List<string>> dict, string text)
        {
            Console.WriteLine("正在寻找所有可能的编码分支...");
            ConcurrentBag<(int, int, string)> tree = [];
            _ = Parallel.For(0, text.Length, i =>
            {
                var branches = dict.Where(x => text[i..(i + limit)].StartsWith(x.Key))
        ...
        private static string[] FindShortest(ConcurrentBag<(int head, int length, string code)> tree, int textLength, int codeID)
        {
            ...
            Console.WriteLine($"共需遍历{textLength}字：");
            for (int i = 1; i < textLength; i++)
            {
                ...
                var shortestWays = tempWays.Where(x => x.Key.Length == mLength).ToArray();
                var suffixes = tree.Where(x => x.head == i).ToArray();
                foreach (var (way, _) in shortestWays)
